...
> Раздел восстановлен на пользовательской установке 10.08 по заметкам к релизу `v0.38.2`: платформа больше не доставляет `CHANGELOG.md`, а сторож выпусков требует совпадения версий (`inbox/bugs/bug-2026-08-10-release-sync-blocks-update-commit.md` в governance-репо).

### Changed

- `guide-kit` обновлён до `v0.1.3`: прикладная дорожка — вход `domain_traits` в планировщик, продакшн-адаптер `profile.yaml` → `HorizonContext`, проброс текста прикладной мини-секции, разложение прикладных источников, персонализация по здоровью (РП-483/495).

### Fixed

- `guide-kit`: планировщик больше не отдаёт занятие без элемента каталога. Добавлен кросс-областной запасной путь — берётся обслуживаемая область с наибольшим остаточным разрывом, поэтому реальный прогресс продолжает направлять выбор.
- `guide-kit`: разложение прикладных источников больше не выбрасывает журнал решений и не падает на объявленном, но пустом списке секций.
