...
5a. **Здоровье платформы (валидация формата)** — секция `<details><summary>Здоровье ...</summary>` ОБЯЗАНА содержать markdown-таблицу с **числовыми ячейками** ИЛИ явный текст «нет данных». Hook regex: `\| *[0-9]|нет данных`. Например:
   ...
   **Числа считать реально, не копировать из примера:** Open Issues = `gh issue list --repo TserenTserenov/FMT-exocortex-template --state open | wc -l` (упало/нет gh → «нет данных»). Писать `0` без выполненной проверки ЗАПРЕЩЕНО — ложный ноль хуже отсутствия данных.
...
- Calendar: попробовать `bash {{WORKSPACE_DIR}}/FMT-exocortex-template/scripts/server-calendar.sh $(date +%F)`. Скрипт недоступен/упал → секцию пометить «Календарь не проверен в автономном прогоне — проверится в интерактивном Day Open». ЗАПРЕЩЕНО писать «Mac-only» или «недоступен на сервере» — это ложный диагноз (календарь доступен через MCP в интерактивной сессии).
