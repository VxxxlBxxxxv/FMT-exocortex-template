...
> **Правило маршрутизации (БЛОКИРУЮЩЕЕ):** по умолчанию знание идёт в **DayPlan как кандидат**, не в captures.md. Пользователь решает в Day Open / в живом разборе — отдавать Экстрактору или нет.
> В `captures.md` писать ТОЛЬКО если в тексте заметки **явно** есть маркер: «Экстрактору», «в captures», «в Pack», «формализовать» (или синонимы).
