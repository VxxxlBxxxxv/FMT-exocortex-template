...
> **Service Clause:** [VR.SC.005-installation-audit](https://github.com/VxxxlBxxxxv/PACK-verification/blob/main/pack/verification/08-service-clauses/VR.SC.005-installation-audit.md)
...
Запустить `bash "$IWE_SCRIPTS/iwe-audit.sh"` (передать `--critical` если в $ARGUMENTS), сохранить вывод в переменную `bash_report`. `$IWE_SCRIPTS` экспортируется из `~/.iwe-env` (генерируется `setup.sh`). Проверить exit code:
- 0 → bash-часть ✅
