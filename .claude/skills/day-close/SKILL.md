...
Если `{{HOME_DIR}}/IWE/${IWE_GOVERNANCE_REPO:-DS-strategy}/scripts/check-index-health.py` существует — запустить его; для каждого FAIL/WARN: диагностика (дамп vs жанр) → перенести или пометить skip. Если скрипт не поставлен — явно записать `Index Health Check: skip (script missing)` и не блокировать Day Close.
...
### 10b. Rule Classifier
Если `$HOME/IWE/.claude/scripts/rule-classifier.py` существует — запустить после коммита (идемпотентно, kill если >60 сек). Если скрипт не поставлен — записать `Rule Classifier: skip (script missing)` и не блокировать Day Close.
...
**EXTENSION POINT (after):** `bash .claude/scripts/load-extensions.sh day-close after` → exit 0: `Read` каждый файл из вывода (alphabetic) → выполнить финальные расширения. Exit 1 → пропустить. Поддерживает `extensions/day-close.after.md` И `extensions/day-close.after.<suffix>.md`.
...
- [ ] **Rule-engine FP-stats** (WP-272 Ф2.5): если `~/IWE/.claude/scripts/fp-stats.py` существует — `python3 ~/IWE/.claude/scripts/fp-stats.py --date $(date +%Y-%m-%d)` → `⚠️ REVISE` → записать в «Завтра начать с»; если скрипта нет — отметить skip, не блокировать Day Close
